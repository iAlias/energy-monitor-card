...
class EnergyMonitorCard extends LitElement {
  ...
  async _detectDevices() {
    ...
    // Rileva sensori di energia automaticamente
    ...
    // Aggiungi dispositivi configurati manualmente
  ...
  async _loadConsumptionData() {
    ...
    try {
      for (const device of this._devices) {
        for (const entity of device.entities) {
          if (entity.type === 'energy') {
            // Carica dati storici
            const consumptionData = await this._getHistoryData(
              entity.entity_id,
              this._startDate,
              this._endDate
            );
            ...
            if (this.config.show_costs) {
              const kwh = this._calculateTotalConsumption(consumptionData);
      ...
      console.error('Errore nel caricamento dati energetici:', error);
    }
  ...
  async _getHistoryData(entityId, startDate, endDate) {
    try {
      const response = await this.hass.callApi(
        'get',
        `history/period/${startDate}T00:00:00Z?end_time=${endDate}T23:59:59Z&entity_ids=${entityId}`
      );

      return response && response[0] ? response[0] : [];
    } catch (error) {
      console.error(`Errore nel caricamento della storia per ${entityId}:`, error);
      return [];
    }
  }
  ...
  _updateComparisonDates() {
    ...
    switch (this._comparisonPeriod) {
      ...
      case 'previous_month':
        const lastMonth = new Date(today);
        lastMonth.setMonth(lastMonth.getMonth() - 1);
        this._comparisonStartDate = this._formatDate(lastMonth);
        this._comparisonEndDate = this._formatDate(lastMonth);
        break;
      case 'custom':
        // Rimane come impostato manualmente
        break;
    }
  }
  ...
  _handleCustomDate(field, e) {
    ...
    } else if (field === 'comparisonStart') {
      this._comparisonStartDate = e.target.value;
    } else if (field === 'comparisonEnd') {
      this._comparisonEndDate = e.target.value;
    }
    this._loadConsumptionData();
  }

  _getComparison(currentValue, comparisonValue) {
    if (comparisonValue === 0 || comparisonValue === undefined) return 0;
        ...
        <div class="card-content">
          ...
          <div class="period-selector">
            <div class="period-row">
              ...
              ${this._selectedPeriod === 'custom' ? html`
                <div class="period-group">
                  <label>Da:</label>
            ...
            ${this.config.show_comparison ? html`
              <div class="period-row">
                <div class="period-group">
                ...
                ${this._comparisonPeriod === 'custom' ? html`
                  <div class="period-group">
                    <label>Da:</label>
                    <input type="date" .value="${this._comparisonStartDate}" 
                      @change="${(e) => this._handleCustomDate('comparisonStart', e)}">
                  </div>
                  <div class="period-group">
                    <label>A:</label>
                    <input type="date" .value="${this._comparisonEndDate}" 
                      @change="${(e) => this._handleCustomDate('comparisonEnd', e)}">
                  </div>
                ` : ''}
              </div>
            ` : ''}
      ...
      <div class="device-card">
        ...
        <div class="device-stats">
          <div class="stat">
            ...
            ${this.config.show_costs && cost ? html`
              <span class="stat-cost">${this._formatCurrency(cost.current)}</span>
            ` : ''}
          </div>

          ${this.config.show_comparison && consumption.comparison ? html`
            <div class="stat">
              <span class="stat-label">Periodo Precedente</span>
              <span class="stat-value">${comparisonKwh.toFixed(2)} kWh</span>
              ${this.config.show_costs && cost ? html`
                <span class="stat-cost">${this._formatCurrency(cost.comparison)}</span>
              ` : ''}
            </div>
  ...
  _renderChart(deviceId, current, comparison) {
    ...
    return html`
      <div class="chart">
        ...
        ${this.config.show_comparison ? html`
          <div class="chart-bar comparison" style="height: ${comparisonHeight}%">
      ...
      <div class="summary">
        ...
        <div class="summary-grid">
          <div class="summary-item">
            ...
            ${this.config.show_costs ? html`
              <span class="summary-cost">${this._formatCurrency(totalCost)}</span>
            ` : ''}
          </div>
          ...
          ${this.config.show_comparison ? html`
            <div class="summary-item">
              ...
              ${this.config.show_costs ? html`
                <span class="summary-cost">${this._formatCurrency(comparisonCost)}</span>
              ` : ''}
            </div>
